...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (opp?.hasDeployed && !prevOpp?.hasDeployed) {
      addNotification(`${opponentColor.toUpperCase()} BATTALION DEPLOYED`, opponentColor);
    }
...
function UnitSelectionModal({ playerColor, onDeploy }) {
  ...
  const totalCost = Object.entries(quantities).reduce((acc, [id, qty]) => {
    return acc + (UNIT_DISPLAY_INFO[id].cost * qty);
  }, 0);
  ...
  const isAffordable = (cost) => remaining >= cost;
    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/90 backdrop-blur-2xl">
      <div className="relative max-w-5xl w-full p-8 rounded-3xl border border-white/10 bg-[#0d1520]/95 shadow-[0_0_100px_rgba(0,0,0,0.8)] flex flex-col h-[85vh]">
        ...
        <div className="flex items-center justify-between mb-8 pb-6 border-b border-white/5">
          <div>
            <p className="text-xs uppercase tracking-[0.4em] text-cyan-400 font-bold mb-1">Logistics & Deployment</p>
            <h2 className="text-3xl font-black text-white">Battalion Composition</h2>
          </div>
          <div className="text-right">
            <p className="text-[10px] uppercase tracking-widest text-slate-500 mb-1">Available Credits</p>
            <div className={`text-3xl font-mono font-black transition-colors ${remaining < 0 ? 'text-rose-500' : 'text-emerald-400'}`}>
        ...
        <div className="grid grid-cols-2 gap-6 overflow-y-auto pr-4 custom-scrollbar mb-8">
          {Object.entries(UNIT_DISPLAY_INFO).filter(([id]) => id !== 'armoredDummy').map(([id, info]) => (
            <div key={id} className="group relative p-6 rounded-2xl border border-white/5 bg-slate-900/40 hover:bg-slate-900/60 transition-all">
              <div className="flex gap-6">
                {/* Visual Icon */}
                <div className={`w-20 h-20 rounded-xl flex items-center justify-center text-2xl font-black border-2 shadow-2xl ${
                  playerColor === 'red' 
                    ? 'border-rose-500/40 bg-rose-500/10 text-rose-300' 
                    : 'border-cyan-400/40 bg-cyan-400/10 text-cyan-300'
                } ${id === 'rifleman' || id === 'antiTank' ? 'rounded-full' : id === 'armoredCar' ? 'rounded-lg' : 'rounded-none'}`}>
                  {info.shortLabel}
                </div>
                ...
                <div className="flex-1">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 mt-1">
                        {info.attributes.map(attr => (
                          <span key={attr} className="px-2 py-0.5 rounded-md bg-white/5 text-[9px] uppercase tracking-wider text-slate-400">{attr}</span>
                        ))}
                      </div>
                    </div>
                    <div className="text-emerald-400 font-mono font-bold">{info.cost} <span className="text-[10px] opacity-50">CR</span></div>
                  </div>
                  
                  <p className="mt-3 text-xs text-slate-400 line-clamp-1">{info.damageDescription}</p>
                  ...
                  <div className="mt-6 flex items-center gap-4">
                    ...
                    <div className="w-12 text-center text-xl font-mono font-black text-white">{quantities[id]}</div>
                    <button 
                      onClick={() => handleUpdate(id, 1)}
